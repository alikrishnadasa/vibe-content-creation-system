...
class ContentSelector:
    ...
    async def select_clips_for_script(self, 
                               ...
                               variation_seed: Optional[int] = None) -> SelectedSequence:
        """
        ...
        # Calculate clip count and timing based on music beats with minimum duration
        if clip_count is None:
            if script_duration is not None and music_beats:
                # Use beat intervals with grouping to ensure minimum clip duration
                relevant_beats = [beat for beat in music_beats if beat <= script_duration]
                
                if len(relevant_beats) >= 2:
                    # Group beats to ensure minimum clip duration
                    grouped_beats = self._group_beats_for_min_duration(relevant_beats, min_clip_duration)
                    clip_count = len(grouped_beats) - 1
                    
                    avg_duration = script_duration / clip_count if clip_count > 0 else min_clip_duration
                    logger.info(f"Beat-synchronized clips with {min_clip_duration:.1f}s minimum: {clip_count} clips (avg {avg_duration:.1f}s) from {len(relevant_beats)} beats")
                else:
        ...
        if script_duration and music_beats_list and len(music_beats_list) >= 2:
            # Use grouped beats for sync points
            grouped_beats = self._group_beats_for_min_duration(
                [beat for beat in music_beats_list if beat <= script_duration], 
                min_clip_duration
            )
        ...
        sync_points = self._generate_music_sync_points(selected_clips, music_beats_list)
    ...
    def _generate_music_sync_points(self, clips: List[ClipMetadata], music_beats: List[float]) -> List[Dict[str, float]]:
        ...
        for i in range(len(clips)):
            if i < len(relevant_beats) - 1:
                ...
                previous_duration = sync_points[-1]['duration'] if sync_points else 2.875
    ...
    def generate_multiple_sequences(self, 
        ...
        if script_duration is not None:
            clips_needed = max(3, int(script_duration / 2.875) + 1)
            target_duration = script_duration
        else:
            clips_needed = 3  # Fallback to 3 clips
            target_duration = 2.875 * clips_needed
