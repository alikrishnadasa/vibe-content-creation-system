...
class FFmpegVideoProcessor:
    ...
    def _build_clip_filter_complex(self, 
        ...
        # Scale and prepare each clip (simplified for speed)
        for i, _ in enumerate(clip_paths):
            filters.append(f"[{i}:v]scale={target_resolution[0]}:{target_resolution[1]}:force_original_aspect_ratio=decrease,pad={target_resolution[0]}:{target_resolution[1]}:(ow-iw)/2:(oh-ih)/2,setpts=PTS-STARTPTS[v{i}]")
        
        # Create concatenation to match exact target duration
        if beat_sync_points and len(beat_sync_points) == len(clip_paths) and target_duration:
            # Use beat-synchronized durations but ensure total matches target
            total_beat_duration = sum(sync_point.get('duration', 2.875) for sync_point in beat_sync_points)
            
            # Scale durations proportionally to match target duration
            duration_scale = target_duration / total_beat_duration if total_beat_duration > 0 else 1.0
            
            for i, sync_point in enumerate(beat_sync_points):
                duration = sync_point.get('duration', 2.875) * duration_scale
        ...
        elif target_duration and len(clip_paths) > 0:
            # Calculate duration per clip to match target duration exactly
            duration_per_clip = target_duration / len(clip_paths)
            ...
            # Fallback: use clips at original duration (may not match audio)
            concat_inputs = "".join(f"[v{i}]" for i in range(len(clip_paths)))
            filters.append(f"{concat_inputs}concat=n={len(clip_paths)}:v=1:a=0[out]")
            logger.warning("No target duration provided - video length may not match audio")
    ...
    async def _try_software_fallback(self, 
        ...
        try:
            ...
            # Output settings
            cmd.extend(["-r", "30", "-an", str(temp_output)])
        ...
        try:
            ...
            if burn_in_captions:
                ...
                cmd = [
                    "ffmpeg", "-y",
                    "-i", video_path,
                    "-vf", f"subtitles={str(srt_path)}:force_style='Fontname=HelveticaTextNow-ExtraBold,Fontsize=24,PrimaryColour=&Hffffff,OutlineColour=&Hffffff,Outline=0,Alignment=2,MarginV=120,Bold=1,Shadow=2,BackColour=&H80000000'",
