...
class RealContentGenerator:
        ...
        try:
            ...
            if request.music_sync:
                ...
                if music_info.get('beats_analyzed'):
                    # Get beats or percussive events for actual script duration
                    if request.use_percussive_sync:
                        music_beats = self.content_database.music_manager.get_beat_timing(
                            0, script_duration, request.sync_event_type
                        )
                        logger.info(f"Using {request.sync_event_type} synchronization: {len(music_beats)} events")
                    else:
                        music_beats = self.content_database.music_manager.get_beat_timing(0, script_duration)
                        logger.info(f"Using beat synchronization: {len(music_beats)} beats")
            ...
            sequence = await self.content_selector.select_clips_for_script(
                ...
                sync_event_type=request.sync_event_type,
                use_percussive_sync=request.use_percussive_sync
            )
    ...
    async def _process_video_with_music(self, 
        ...
        music_params = self.content_database.music_manager.prepare_for_mixing(
            sequence.total_duration, 
            request.sync_event_type if request.use_percussive_sync else 'beat'
        )
